...
**Anchored comparative-residual calibration for LLM4Rec.** Final score `score_i = pony_i + λ·β_i`, λ≥0 selected on validation (λ=0 allowed ⇒ floor = pony's pointwise posterior). `β_i` is the comparative correction a frozen Qwen3-8B makes only under forced pairwise comparison, estimated via a **ridge-regularized** Bradley–Terry (BT) field (L2 penalty on β, mean-centered; the L2 prior resolves sparse/disconnected-graph rank-deficiency — see `bt_field.py`) over O(K log K) cheap, vLLM-batchable duels anchored at pony's posterior. Question: does that comparative residual carry top-k ranking signal **not captured by this pointwise posterior (same frozen LLM + prompt family)**, lifting NDCG@10 at fixed compute? Plus a scientific sub-finding: is the LLM's pairwise preference field non-BT (significant cyclic intransitivity)?
...
Position-bias controls (symmetrized + neutral labels + title-truncation + repeated-duel CIs on s_ij). **β-identifiability-under-sparsity safeguards (pre-registered, per the method §3 caveat):** (a) report `α` stability and `β` norm across **reseeded duel schedules**; (b) enforce a **per-item minimum duel-coverage floor** (each candidate appears in ≥ a fixed number of duels before its `θ_i` is trusted); (c) a **control regressing `β_i` on per-item duel count/graph degree** to confirm the residual is NOT explained by sampling density (if it is, `β` is a sampling artifact → demote). These directly answer the "expensive adaptively-sampled noisy residualizer" objection. **What is seeded (≥20 seeds for paper-result rows):** the LLM forward pass is *deterministic* (vLLM greedy, temp→0), so we do NOT average over model randomness; the meaningful stochastic sources we seed are (a) the adaptive **duel subsampling / merge-sort scheduling**, (b) the **λ/α validation-selection bootstrap**, and (c) **prompt-order/label perturbations** (A/B position, label tokens). Report mean ± CI over these. Optional backbone transfer reusing pony's Mistral/Llama scores as anchors.
